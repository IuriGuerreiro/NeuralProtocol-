...
class LLMClient:
    """Manages communication with the Gemini LLM provider."""
    ...
    def get_response(self, messages: list[dict[str, str]]) -> str:
        ...
        Raises:
            Exception: If the request to Gemini fails.
        """
        try:
...
class ChatSession:
                ...
                for server in self.servers:
                    try:
                        ...
                        if any(tool.name == tool_name for tool in tools):
                            logging.info(f"✅ Found tool {tool_name} on server {server.name}")
                        ...
                        logging.error(f"❌ Error executing tool {tool_name} on server {server.name}: {e}")
                        continue

                logging.warning(f"⚠️ Tool {tool_name} not found on any server")
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            system_message = (
                "You are a helpful Gemini AI assistant with access to these tools:\n\n"
                f"{tools_description}\n"
                ...
                "}\n"
                "```\n\n"
                "The system will automatically execute the tool when you provide this format.\n\n"
